package com.ace.mobile.feature.exercise.service.worker
...
import com.ace.mobile.feature.exercise.domain.SendPendingBlocksUseCase
...
class SyncBlockWorker @AssistedInject constructor(
    ...
    private val rankingCacheRepository: RankingCacheRepository  // ← NUEVO
        ...
        return try {
            when (val useCaseResult = sendPendingBlocksUseCase(sessionId)) {
                is SendPendingBlocksUseCase.Result.Success -> {
                    ...
                    // ← NUEVO: Invalidar cache de ranking si hubo bloques syncados
                    // (El backend recalcula ranking cada hora, pero si el sync
                    //  fue exitoso, forzamos refresh en próxima apertura)
                    if (useCaseResult.syncedCount > 0) {
                        rankingCacheRepository.invalidateCache()
                        Log.i(TAG, "Ranking cache invalidated after successful sync")
                    }
