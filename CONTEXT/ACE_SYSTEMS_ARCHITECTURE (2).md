...
> **Nota:** Ninguna decisión aquí es verdad por sentada. Este documento es el punto de partida para explorar cada sistema en profundidad en conversaciones futuras.
...
**Principio rector:** *El reloj captura, el móvil calcula y transporta, el backend valida y decide, la base de datos recuerda.*

---

## 2. Sistemas Fundamentales (Índice)

| # | Sistema | Descripción en una línea |
|---|---------|--------------------------|
| 1 | **Captura de Sensor** | Del sensor de FC del reloj al buffer del móvil vía Bluetooth. |
| 2 | **Sesión de Ejercicio** | Ciclo de vida de una actividad: inicio, pausas, bloques, cierre. |
| 3 | **Sincronización Offline-First** | Cómo la APK guarda, encola y envía datos (crudos + XP calculada) al backend. |
| 4 | **Autenticación JWT Híbrida** | Tokens de corta y larga duración, rotación, revocación y race conditions. |
| 5 | **Cálculo de XP y Gamificación** | La APK calcula XP localmente usando fórmulas del backend. Recompensa inmediata offline. |
| 6 | **Ranking y Posicionamiento** | Clasificación global y municipal, recálculo periódico, cache en APK. |
| 7 | **Racha (Streaks)** | Hábito diario: detección, incremento, rotura y recordatorios. Controlado por backend. |
| 8 | **Notificaciones y Recordatorios** | WorkManager, foreground service y engagement del usuario. |
| 9 | **Historial de Sesiones** | Las últimas 5 sesiones guardadas localmente en la APK, sin discriminar categoría. |
| 10 | **Estadísticas Persistentes de Perfil** | Datos procesados (con XP) del usuario, sincronizados y cacheados en la APK. |
...
|--------|----------------|-------------|
| **Wear OS** | Activar sensor a 1 Hz. Enviar `{"bpm": int, "timestamp": long}` por DataClient en path `/ace/heart_rate`. Escuchar comandos `START`/`STOP` del móvil. | No agrupa en bloques. No sabe de sesiones. No calcula promedios. No tiene lógica de negocio. |
...
### Contratos clave
- **Path DataClient:** `/ace/heart_rate` (FC en vivo), `/ace/session/{id}/status` (comando STOP).
...
- `PENDING`: Listo para enviar. XP ya calculada y visible para el usuario.
- `SYNCING`: En vuelo (evita doble envío si el usuario cierra la app).
- `SYNCED`: Backend confirmó con `201`. XP es definitiva.
- `ERROR`: Falló 5 reintentos o backend rechazó XP como inválida. Requiere acción manual o diagnóstico.
